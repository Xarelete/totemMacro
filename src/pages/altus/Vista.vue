      ...
      <div class="mt-10 grid gap-10 lg:grid-cols-[1.1fr_1fr] items-center">
        ...
        <Transition name="item" appear>
          <div class="flex items-center justify-center overflow-hidden rounded-3xl border border-white/15 bg-white/10 backdrop-blur-sm shadow-2xl">
            <img :src="torreNoturna" alt="Vista noturna da torre do Altus América" class="w-full h-auto object-contain" :style="{ maxHeight: 'calc(100vh - 16rem)' }" />
          </div>
        </Transition>
      </div>
    </div>

    <div v-if="showModal" class="fixed inset-0 z-50 flex items-center justify-center p-4" @keydown.esc.window="closeModal" role="dialog" aria-modal="true" aria-label="Tour 360 Altus América" tabindex="-1">
      ...
      <div class="relative z-10 w-full max-w-5xl overflow-hidden rounded-3xl border border-white/20 shadow-2xl">
        ...
        <iframe :src="kuulaUrl" title="Tour 360 Altus América" class="aspect-video w-full bg-black" allow="xr-spatial-tracking; gyroscope; accelerometer" allowfullscreen></iframe>
      </div>
    </div>
  </section>
</template>
